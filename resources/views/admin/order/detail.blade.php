...
<div class="row">
    ...
    @if($users->role != 2 && $users->role != 4) <td style="width: 1%;text-align: center;"> <input type="checkbox"  name="traDo" data-stt="{{$key+1}}" value="{{$item->id}}"></td>@endIF
        </tr>
        @endforeach
        <form >
        <tr>
          <td><button type="button" class="btn btn-block bg-gradient-danger" onclick="deleteAll()">Xóa toàn bộ</button></td>
            ...
            <td> 
              ...
              <button type="button" class="btn btn-success swalDefaultSuccess"  onclick="traDon()">
                Trả toàn bộ đơn
                </button>
                @endIF
           </td>
...
function traDon(){
  if(confirm("Bạn muốn trả toàn bộ đơn "+{{$order->id}}) == true){
    id={{$order->id}};
    var _token = $('input[name="_token"]').val();
    $.ajax({ 
      headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
              ...
              datatype: 'JSON',
              data: {id:id},
              url: '{{URL::to('/order/finish/allDetail/')}}',
              success: function(result){
                 if(result.error == false){
...
function deleteDetail(event){
  ...
  if(confirm("Bạn muốn xóa đồ uống thứ "+stt) == true){
    $.ajax({
              ...
              url: '{{URL::to('/order/delete/detail')}}',
              success: function(result){
                 if(result.error == false){
...
function deleteAll(){
  ...
  if(confirm("Bạn muốn xóa tất cả đồ uống ?") == true){
    $.ajax({
              ...
              url: '{{URL::to('/order/deleteAll/detail')}}',
              success: function(result){
                 if(result.error == false){
...
<script type="text/javascript">
  ...
  var stt_Detail = [];
  var _token = $('input[name="_token"]').val();
  // Duyệt qua các checkbox và lưu trữ giá trị của các checkbox được chọn
  checkboxes.forEach(function(checkbox) {
    if (checkbox.checked) {
      id_Detail.push(checkbox.value);
      stt_Detail.push(checkbox.getAttribute('data-stt'));
    
    }
  });
  if(id_Detail.length === 0){
       alert('Hãy chọn đồ uống để trả đồ ');
    ...
    var _token = $('input[name="_token"]').val();
      $.ajax({
                method: 'POST',
                datatype: 'JSON',
                data: {id:id,id_Detail:id_Detail,stt_Detail:stt_Detail,_token:_token},
                url: '{{URL::to('/order/finish/detail')}}',
                success: function(result){
                   if(result.error == false){
  ...
  function updateQty(event){
      ...
      $.ajax({
                ...
                url: '{{URL::to('/order/update/detail')}}',
                success: function(result){
                   if(result.error ==false){
    ...
    Echo.private('finishDetails')
      ...
      if(e.o.id == {{$order->id}}){
        ...
        if(e.mess.status === '0'){
            ...
            title: "Đã trả toàn bộ đơn "+e.o.id,
            icon: "success",                         
            confirmButtonText: "Đi tới danh sách đơn hàng"                  
            }).then((result) => {
       /* Read more about isConfirmed, isDenied below */
            ...
            window.location.href = "{{url('/order/bill/list')}}";   
                      ...
                      title: "Đã trả đồ một phần đơn "+e.o.id,
                      icon: "success",                         
          ...
          var stt = document.getElementsByClassName("sttDetail");
      // Duyệt qua từng dòng và xóa
          for(var i=0;i<e.mess.stt.length;i++){
          
             table.deleteRow(e.mess.stt[i]);
          }  
              for(var j=0;j<stt.length;j++){
                  stt[j].innerHTML = j+1;
                    }
                          });
              var b = document.getElementById("cancleButton1");
