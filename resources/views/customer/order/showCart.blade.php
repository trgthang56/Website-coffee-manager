...
<div class="app pt-80 pb-70">
    ...
    @elseIf(empty($cart))
    <h1 style="text-align: center"> <i class="fas fa-weight-hanging"></i> &nbsp;Giỏ hàng trống </h1>
    @endIf
</div>
          ...
          function ChangeQty(event){    
            ...
            if(event.getAttribute('data-name') === 'pluss'){
             ...
             $.ajax({
              ...
              url: '{{URL::to('/customer/update/cart')}}',
              success: function(result){
                 if(result.error ==false){
                    location.reload();
            ...
            function order(){
                ...
                $.ajax({
              method: 'POST',
              data: {table_id:table_id,mess_bill:mess_bill,_token:_token},
              url: '{{URL::to('/customer/create/bill/')}}',
              success:function(data){
                    ...
                    confirmButtonText: "Đi tới trang chủ"                  
                     ...
                     if (result.isConfirmed) {
                        window.location.href = "{{url('/customer/order')}}"+'/'+{{$table->id}};
